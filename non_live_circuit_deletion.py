...
class NonLiveCircuitDeletion:
    ...
    def __init__(self, driver, db_query, diag_username, diag_passwords, filepath):
    ...
    def nms_level_deletion(self, row, index, processed_db_ids, connection_id, tjs_id, service_name):
        ...
        if is_unknown_node:
            ...
            node, port = self.get_vcg_node_and_port(row)
            ...
            if node and port and not is_duplicate_vcg:
                logger.info(f"VCG/E1 found -> Node: {node}, Port: {port}")
                time.sleep(1)

                flow_status = self.check_traffic_flow(node, port)
    ...
    def check_traffic_flow(self, node, port):
    ...
    def get_vcg_node_and_port(self, row):
        ...
        # if any(x in a_port for x in ["VCG", "E1"]):
        #     return row.get("a_end"), a_port
        # if any(x in b_port for x in ["VCG", "E1"]):
        #     return row.get("b_end"), b_port
        if "E1" in a_port:
            return row.get("a_end"), a_port
        if "E1" in b_port:
            return row.get("b_end"), b_port
    ...
    def apply_cross_connect_filter(self, node, card, port, stm, jklm, connection_id, full_port, is_priority=False):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 10)
    ...
    def l2_eth_card(self, port_number, node, vcg_flow):
        ...
        logger.info(f"L2 ETH|packet flow of port 5: {packet_flow}")
        eth_status["packet_flow"] = packet_flow
    ...
    def l1_eth_card(self, port_number, vcg_flow):
        ...
        if eth_ad_status == 'UP' and eth_oper_status == 'DOWN':
            ...
            elif vcg_ad_status == 'UP' and vcg_oper_status == 'UP':
                cir_st = "live"
                logger.info(
                    f"L1_ETH: ETH: {eth_ad_status}|{eth_oper_status} VCG: {vcg_ad_status}|{vcg_oper_status} | Live")
                result["cir_status"] = cir_st
                result["alarm_info"] = "NA"
                # return result
        else:
    ...
    def admin_oper_alarm(self, port_name):
        ...
        if admin_status.strip().upper() == "UP" and operational_status.strip().upper() == "DOWN":
            ...
            logger.info(f"E1 port {port_number} admin:{admin_status},operational:{operational_status}|Live")
            cir_st = "live"
            result["cir_status"] = cir_st
            result["alarm_info"] = "NA"
            return result
    ...
    def main_process(self):
        ...
        self.devices_df = self.db_query.get_ais_circuit_data(self.filepath)
        ...
        for index, row in self.devices_df.iterrows():
            ...
            db_id = self.db_query.get_non_live_row(connection_id)
            processed_db_ids.append(str(db_id))
